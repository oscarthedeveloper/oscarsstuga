...
// Behåll accenter (betydelsebärande), men strippa apostrofer, gemener och trim.
function norm(s) {
...
const MOODS = ['Indikativ', 'Konditionalis', 'Konjunktiv', 'Imperativ'];
const EMPTY6 = ['', '', '', '', '', ''];

export default function SpanishVerbDrill() {
  ...
  const pillLabel = tense.mood === 'Imperativ' ? 'Imperativo' : `${tense.mood} · ${tense.label}`;
  ...
  return (
    <div className={styles.drill}>
            ...
            {MOODS.map((m) => (
              <optgroup key={m} label={m}>
                {TENSES.filter((t) => t.mood === m).map((t) => (
                  <option key={t.key} value={t.key}>{t.imperative ? 'Imperativo' : t.label}</option>
                ))}
              </optgroup>
            ))}
          </select>
      ...
      <div className={styles.head}>
        ...
        <div className={styles.headSub}>{verb.sv} · {verb.cat.toLowerCase()}</div>
      </div>
